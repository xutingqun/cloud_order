...
public class RollHeaderView extends FrameLayout implements OnPageChangeListener {
	...
	public void setImgUrlData(List<String> urlList) {
		...
		if (mUrlList != null && !mUrlList.isEmpty()) {
			...
			for (int i = 0; i < mUrlList.size(); i++) {
				...
				if (i == 0) {
					dotIv.setBackgroundResource(R.drawable.jshop_banner_point_active);
				} else {
					dotIv.setBackgroundResource(R.drawable.jshop_banner_point_inactive);
				}
				// 璁剧疆鐐圭殑闂磋窛
	 ...
	 */
	public void setOnHeaderViewClickListener(
	...
	private class MyAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, final int position) {
			...
			iv.setOnTouchListener(new OnTouchListener() {
				...
				public boolean onTouch(View v, MotionEvent event) {
					...
					case MotionEvent.ACTION_UP:
						...
						if (downX == moveX && (moveTime - downTime < 500)) {// 鐐瑰嚮鐨勬潯锟�?
							// 杞挱鍥惧洖璋冪偣鍑讳簨锟�?
							if(headerViewClickListener!=null){
								headerViewClickListener.HeaderViewClick(position
										% mUrlList.size());
							}
						}
						break;
			...
			Glide.with(mContext).load(mUrlList.get(position % mUrlList.size()))
					.error(R.drawable.mo_cal)
					.placeholder(R.drawable.mo_cal)
	...
	public void onPageSelected(int position) {
		dotList.get(prePosition).setBackgroundResource(
				R.drawable. jshop_banner_point_inactive);
		dotList.get(position % dotList.size()).setBackgroundResource(
				R.drawable.jshop_banner_point_active);
